...
class LayerNormLSTM(nn.Module):
    ...
    def forward(self, input, hidden=None):
        ...
        ht = [[None, ] * (self.num_layers * num_directions)] * seq_len  # seq_len * (num_layers * num_directions)
        ct = [[None, ] * (self.num_layers * num_directions)] * seq_len  # seq_len * (num_layers * num_directions)
        ...
        if self.bidirectional:
            ...
            for l, (layer0, layer1) in enumerate(zip(self.hidden0, self.hidden1)):
                ...
                for t, (x0, x1) in enumerate(zip(xs, reversed(xs))):    # xs: seq_len, batch_size, hidden_size, so x0, x1: B * H
